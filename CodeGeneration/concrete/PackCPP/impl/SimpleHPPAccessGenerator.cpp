...
namespace dsg {
	...
	namespace generator {
		...
		SimpleHPPAccessGenerator::SimpleHPPAccessGenerator()
		{
			...
			CreatePrivateMembers_R2 =
                   "\tstd::vector< shared_ptr< " << EntityClass_R2 << " > > m_backup;"               "\r\n"
			...
			CreatePublicMembers_R2 =
                ...
                << "\t std::vector< shared_ptr< " << EntityClass_R2 << " > > getMany" << UEntity_R2 << "s(typename EncodingT::string_t const&  filter) const;\r\n"
				<< CommentAccessGetAll_R2
				<< "\t std::vector< shared_ptr< " << EntityClass_R2 << " > > getAll" << UEntity_R2 << "s() const;\r\n"
				<< CommentAccessGetOne_R2   
				<< "\t shared_ptr< " << EntityClass_R2 << " > getOne" << UEntity_R2 << "(" << AttributeIDParam_R2 << ") const;\r\n"
				<< CommentAccessSelectMany_R2
				<< "\t std::vector< shared_ptr< " << EntityClass_R2 << " > > selectMany" << UEntity_R2 << "s(typename EncodingT::string_t const&  filter, bool nowait = false, bool addition = false);\r\n"
				<< CommentAccessSelectOne_R2
				<< "\t shared_ptr< " << EntityClass_R2 << " > selectOne" << UEntity_R2 << "(" << AttributeIDParam_R2 << ", bool nowait = false, bool addition = false);\r\n"
				<< CommentAccessIsSelected_R2
                << "\t bool isSelected" << UEntity_R2 << "(shared_ptr< " << EntityClass_R2 << " > o) const;\r\n"
				<< CommentAccessCancel_R2
				<< "\t void cancelSelection();\r\n"
				<< (( CommentAccessFill1_R2
                << "\t void fill" << UAttribute_R2 << "(shared_ptr< " << EntityClass_R2 << " > o);\r\n")
																							[If_IsREF_R2 && !If_IsID_R2])        
																							[attr0, attrN]
				<<  ListRelationN_R2( CommentAccessFill2_R2
                <<  "\t void fillAll" << URelationNName_R2 << "s(shared_ptr< " << Relation1Class_R2 << " > o, bool nowait = false);\r\n"
				<<  CommentAccessFill3_R2
                <<  "\t void fillOne" << URelationNName_R2 << "(shared_ptr< " <<  Relation1Class_R2 << " > o"
                ...
                <<  "\t void fillMany" << URelationNName_R2 << "s(shared_ptr< " << Relation1Class_R2 << " > o, typename EncodingT::string_t const&  filter, bool nowait = false);\r\n")
				<<  CommentAccessAltered_R2
				<<  "\t bool isModified" << UEntity_R2 << "(shared_ptr< " << EntityClass_R2 << " > o) const;\r\n"
				<<  CommentAccessUpdate_R2
				<<  "\t void update" << UEntity_R2 << "(shared_ptr< " << EntityClass_R2 << " > o);\r\n"
				<<  CommentAccessInsert_R2
				<<  "\t void insert" << UEntity_R2 << "(shared_ptr< " << EntityClass_R2 << " > o);\r\n"
				<<  CommentAccessDelete_R2
                <<  "\t void delete" << UEntity_R2 << "(shared_ptr< " << EntityClass_R2 << " > o" << str_g(", bool deleteid = true")[If_HasREF_ID_R2 && If_HasUniqueID_R2] << ");\r\n"
