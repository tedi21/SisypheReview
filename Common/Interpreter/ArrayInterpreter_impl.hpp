...
NAMESPACE_BEGIN(interp)
    ...
    shared_ptr< Base<EncodingT> > ArrayOperator<EncodingT>::interpret(Context<EncodingT> & c)
    {
        shared_ptr< Base<EncodingT> >  index = m_index->interpret(c);
        shared_ptr< Base<EncodingT> >  var   = m_name->interpret(c);
    ...
    template <class EncodingT>
    void ArrayOperator<EncodingT>::allocate(shared_ptr< Base<EncodingT> > const& value, Context<EncodingT> & c)
    {
        shared_ptr< Base<EncodingT> >  index = m_index->interpret(c);
        shared_ptr< Base<EncodingT> >  var   = m_name->interpret(c);
        std::vector<shared_ptr< Base<EncodingT> > > params;
    ...
    template <class EncodingT>
    bool ArrayOperator<EncodingT>::parse(typename EncodingT::string_t const& buf, shared_ptr< Term<EncodingT> > & value)
    {
        typename EncodingT::string_t expr = eat_space<EncodingT>(buf);
        shared_ptr< Term<EncodingT> > expr_value;
        shared_ptr< Address<EncodingT> > arr_value;
