...
_DataStatement<EncodingT>::setText		(int index, const typename ucs::string_t& value)
{
	if (!m_statement)
	{
        ...
        return sqlite3_bind_text(m_statement, index, enc::encode<ucs,dbs>(value).c_str(), -1, SQLITE_TRANSIENT) == SQLITE_OK;
}
...
_DataStatement<EncodingT>::getText		(int column, typename ucs::string_t& value) const
{
	if (!m_statement)
	{
        ...
        value = enc::encode<dbs,ucs>(reinterpret_cast<const typename dbs::char_t*>(sqlite3_column_text(m_statement, column)));
	return true;
}
...
_DataStatement<EncodingT>::getText		(int column, typename ansi::string_t& value) const
{
	if (!m_statement)
	{
        ...
        value = reinterpret_cast<const typename ansi::char_t*>(sqlite3_column_text(m_statement, column));
	return true;
}
