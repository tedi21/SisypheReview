...
template <class EncodingT>
IDiaTablePtrInterpreter<EncodingT>::IDiaTablePtrInterpreter(const IDiaTablePtrInterpreter<EncodingT>& rhs)
{
	initValue(rhs.refValue());
}

template <class EncodingT>
IDiaTablePtrInterpreter<EncodingT>& IDiaTablePtrInterpreter<EncodingT>::operator=(const IDiaTablePtrInterpreter<EncodingT>& rhs)
{
	value(rhs.refValue());
	return *this;
}
...
boost::shared_ptr< Base<EncodingT> > IDiaTablePtrInterpreter<EncodingT>::invoke(const typename EncodingT::string_t& method, std::vector< boost::shared_ptr< Base<EncodingT> > >& params)
{
	...
	{
		if (tryInvoke(this, C("IDiaTablePtr"), method, args, ret) ||
