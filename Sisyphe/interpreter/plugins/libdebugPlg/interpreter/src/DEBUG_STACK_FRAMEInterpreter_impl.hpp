...
template <class EncodingT>
DEBUG_STACK_FRAMEInterpreter<EncodingT>::DEBUG_STACK_FRAMEInterpreter(const DEBUG_STACK_FRAMEInterpreter<EncodingT>& rhs)
{
	initValue(rhs.refValue());
}

template <class EncodingT>
DEBUG_STACK_FRAMEInterpreter<EncodingT>& DEBUG_STACK_FRAMEInterpreter<EncodingT>::operator=(const DEBUG_STACK_FRAMEInterpreter<EncodingT>& rhs)
{
	value(rhs.refValue());
	return *this;
}
...
boost::shared_ptr< Base<EncodingT> > DEBUG_STACK_FRAMEInterpreter<EncodingT>::invoke(const typename EncodingT::string_t& method, std::vector< boost::shared_ptr< Base<EncodingT> > >& params)
{
	...
	{
		if (tryInvoke(this, C("DEBUG_STACK_FRAME"), method, args, ret) ||
