...
NAMESPACE_BEGIN(interp)
	...
	public:
		...
		WordXMLNodeListInterpreter(const WordXMLNodeListInterpreter<EncodingT>& rhs);

		WordXMLNodeListInterpreter<EncodingT>& operator=(const WordXMLNodeListInterpreter<EncodingT>& rhs);

		const WordXMLNodeList& getValue() const;

		void setValue(WordXMLNodeList const& object);
		...
		FACTORY_PROTOTYPE1(item, In< shared_ptr< Base<EncodingT> > >)
		shared_ptr< Base<EncodingT> > item(const shared_ptr< Base<EncodingT> >& i) const;

		// Retourne la taille de la liste.
		shared_ptr< Base<EncodingT> > length() const;

		FACTORY_BEGIN_REGISTER
			CLASS_KEY_REGISTER  ( WordXMLNodeListInterpreter, C("WordXMLNodeList") );
			METHOD_KEY_REGISTER1( WordXMLNodeListInterpreter, shared_ptr< Base<EncodingT> >, item, const_t, C("WordXMLNodeList::Item") );
			METHOD_KEY_REGISTER ( WordXMLNodeListInterpreter, shared_ptr< Base<EncodingT> >, length, const_t, C("WordXMLNodeList::Length") );
	...
	template <class EncodingT>
	bool check_WordXMLNodeList(shared_ptr< Base<EncodingT> > const& val, WordXMLNodeList& a);

	template <class EncodingT>
	bool reset_WordXMLNodeList(shared_ptr< Base<EncodingT> >& val, WordXMLNodeList const& a);
